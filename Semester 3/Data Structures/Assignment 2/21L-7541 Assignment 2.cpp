...
public:
	...
	friend ostream& operator<<(ostream& ob,coordinates a) {
		cout << "(" << a.x << "," << a.y << ")" << endl;
		return ob;
	}
};
...
public:
	...
	bool top(int& dat) {
		if (head->next->next != tail) {
			dat = head->next->next->data.roomno;
			return 1;
		}
		return 0;
	}
...
class prevroom {
public:
	stack<coordinates>prmovs;//previous move
	int roomno;
public:
	prevroom() {
		roomno = 0;
	}
	void operator=(const prevroom& a){
		roomno = a.roomno;
		prmovs = a.prmovs;
	}
	void push(coordinates a) {
		prmovs.push(a);
	}
	int getsize() {
		return prmovs.getsize();
	}
	friend ostream& operator<<(ostream &ob,prevroom a) {
		cout << "Room no: " << a.roomno+1 << endl;
		a.prmovs.print();
		return ob;
	}
	void setroom(int a) {
...
class hauntedhouses {
	...
	string* roomno;
	char** roomcells;
	stack<prevroom>prroom;//previous room
	int croom;
	prevroom prmov;
	coordinates curr;
	coordinates total;
