...
public:
	...
	void printdetail(node* x) {
		...
		cout << x->refbook << " copies available as a reference book\n";
		
		cout << x->issuablebook << " copies available as a issuable book\n";
		cout << "\n\n";
		
	}
	void insertdetail(node*& x) {//detail of book
		cin.ignore();
...
int main() {
	AVL a;
	int x;
	cout << "Welcome to Fast Library\n";
	
	while (1) {
		cout << "Press 1)For Create New record of book\nPress 2)For review a book record\nPress 3)For Update a book record\n"
			"Press 4)For Delete a book record\nPress -1)For Quit\n";
		cout << "\nSelect from above\n";
		cin >> x;
		if (x == 1) {
			a.insert();
	...
	cout << "Thanks For Using FAST Library\n";
}